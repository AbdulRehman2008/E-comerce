  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Take only the first 4 women's clothing items
        const selectedProducts = data.slice(0, 4).map(item => ({
          id: item.id,
          name: item.title,
        ...
        if (selectedProducts.length === 0) {
          throw new Error('No women\'s clothing products found');
        }
    ...
    <section className="py-16 md:py-24">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 md:mb-16">
          <h2 className="text-3xl md:text-4xl font-light mb-4 text-gray-900">Women's Collection</h2>
          <p className="text-gray-600 text-sm md:text-base font-light max-w-2xl mx-auto">
            Discover our latest collection of women's fashion essentials.
          </p>
        </div>
